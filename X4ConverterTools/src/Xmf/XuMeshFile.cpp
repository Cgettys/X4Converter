...
XuMeshFile* XuMeshFile::ReadFromFile(const std::string& filePath,
...
XuMeshFile* XuMeshFile::ReadFromIOStream(IOStream* pStream) {
	if (!pStream) {
		// TODO exception?
		std::cerr << "Warning: no IOStream provided";
		return new XuMeshFile();
	}
	try {
		XuMeshFile *pMeshFile = new XuMeshFile();
