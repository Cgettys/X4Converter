...
void XmfFile::ReadBuffers(StreamReaderLE &pStreamReader) {
  for (XmfDataBuffer &buffer : buffers) {
    buffer.ReadBinary(pStreamReader);
  }
}

std::string XmfFile::Validate() {
  std::string result = header.validate();
  int numVertices = -1;
  for (XmfDataBuffer &buffer : buffers) {
...
void XmfFile::WriteBufferDescs(StreamWriterLE &pStreamWriter,
  ...
  for (XmfDataBuffer &buffer: buffers) {
    ...
    buffer.Description.WriteBinary(pStreamWriter);
...
void XmfFile::AllocMeshVertices(aiMesh *pMesh, int numVertices) {
  ...
  for (XmfDataBuffer &buffer : GetBuffers()) {
    ...
    for (auto i = 0UL; i < buffer.Description.NumVertexElements; ++i) {
...
void XmfFile::PopulateMeshVertices(aiMesh *pMesh, uint32_t firstIndex, uint32_t numIndices) {
  ...
  for (XmfDataBuffer &buffer : GetBuffers()) {
    ...
    for (unsigned int elemIdx = 0; elemIdx < buffer.Description.NumVertexElements; ++elemIdx) {
...
XmfFile::GenerateMeshFile(const ConversionContext::Ptr &ctx, aiNode *pNode, bool isCollisionMesh) {
  ...
  for (aiNode *pMeshNode: meshNodes) {
    ...
    for (uint32_t i = 0; i < pMesh->mNumFaces; ++i) {
      ...
      for (int j = 0; j < 3; ++j) {
        unsigned int index = vertexOffset + pFace->mIndices[j];
        if (indexBuffer.Description.ItemSize == sizeof(uint16_t)) {
          *(uint16_t *) pIndex = numeric_cast<uint16_t>(index);
        } else {
          *(int *) pIndex = numeric_cast<int>(index);
        }
