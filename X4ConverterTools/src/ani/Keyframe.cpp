...
*/
    ...
    float Keyframe::getValueByAxis(std::string axis) {
        if (axis == "X") {
            return ValueX;
        } else if (axis == "Y") {
            return ValueY;
    ...
    InterpolationType Keyframe::getInterpByAxis(std::string axis) {
        if (axis == "X") {
    ...
    std::pair<float, float> Keyframe::getControlPoint(std::string axis, bool in) {
