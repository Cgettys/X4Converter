struct XmfFile {
  ...
  int32 primitiveType =
      4;                // byte 22 - 25 [suspect, not aligned - is this actually a byte?] D3DPRIMITIVETYPE; determines how the index buffer is used to draw polygons. 4 = triangle list
  ...
  uint8_t bufferData[compressedDataSize
  ]; // content of the DirectX vertex/index buffer, may be zlib-compressed. if bCompressed = 1,
...
struct DataBufferDesc {
  ...
  int32
      usageIndex;               // bytes 4 - 7   for vertex buffers with numVertexElements = 0, indicates the usage index of the single implicit element
  int32
      dataOffset;               // bytes 8 - 11  file offset = dataBufferDescOffset + numDataBuffers*dataBufferDescSize + numAttributeRanges*attributeRangeSize + dataOffset
  int32
      bCompressed;              // bytes 12 - 15 if 1, the data is compressed using zlib's compress() function and can be uncompressed with uncompress()
  uint8_t pad[4];                    // bytes 16 - 19 [padding?]
  int32
      format;                   // bytes 20 - 23 for index buffers, this is the index format: 0x1E -> 16-bit indices, otherwise 32-bit indices.
  ...
  int32
      numVertexElements;        // bytes 56 - 59 for the vertex buffer, number of elements in the DirectX vertex declaration. May be 0,
  // in which case a single declaration is built using the preceding fields.
  VertexDeclElement elements[16
  ]; // bytes fixed size array, only the first numVertexElements items are used. each item is mapped to a D3DVERTEXELEMENT9
};
...
struct AttributeRange {
  ...
  char name[128
  ]; // bytes 8 - 135; sometimes materialCollection + "." + materialName; actual material for the mesh is defined in the mesh .xml in <part>
